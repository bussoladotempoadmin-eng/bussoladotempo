...
export async function gerarAgendaIA(
  ...
  const system = [
    ...
    '- PRESERVE A GRANULARIDADE: cada atividade distinta é um bloco PRÓPRIO com seu título. NUNCA junte atividades diferentes num bloco só nem empilhe títulos. Replique a estrutura de blocos que o usuário já usa.',
...
export async function revisarEPlanejar(
  ...
  const system = [
    ...
    '- PRESERVE A GRANULARIDADE do histórico: cada ATIVIDADE distinta é um bloco PRÓPRIO com seu título (ex.: "Abertura do dia", "Análise de desempenho", "Relatório semanal" são blocos SEPARADOS, um por horário). NUNCA junte atividades diferentes num bloco só, nem empilhe vários títulos num único bloco. Replique a estrutura de blocos que o usuário já usa. Só evite quebrar uma MESMA atividade contínua em pedacinhos.',
